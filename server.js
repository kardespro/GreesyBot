                    ...
                    if (!time) {
                        message.channel.send(
                            new Discord.MessageEmbed()
                                ...
                                .setColor("RED")
                                .setDescription(`The \`First\` Argument Must be a Number.`)//**first** bağımsız değişkeni bir sayı olmalıdır. 
                        );
                    } else {
                        if (time < 5000 || time > 2678400000) {
                            message.channel.send(
                                new Discord.MessageEmbed()
                                    .setTitle('Greesy | Giveaway')
                                    .setColor("RANDOM")
                                    .setDescription(`The \`First\` Argument Cannot be less than 5 **Second(s) ** or Greater than 30 **Day(s) **.`)//**first** bağımsız değişkeni 5 saniyeden az veya 31 günden büyük olamaz
                            );
                        } else {
                            if (isNaN(winners)) {
                                message.channel.send(
                                    new Discord.MessageEmbed()
                                        .setTitle('Greesy | Giveaway')
                                        .setColor("RED")
                                        .setDescription(`The \`second\` Argument must be a Number.`) //**second** bağımsız değişkeni bir sayı olmalıdır
                                );
                            } else {
                                if (winners < 1 || winners > 10) {
                                    message.channel.send(
                                        new Discord.MessageEmbed()
                                            .setTitle('Greesy | Giveaway')
                                            .setColor("RED")
                                            .setDescription(`The Winning Number must be between \`1\` and \`10\`! `)//**second** bağımsız değişkeni 1'den az veya 10'dan büyük olamaz
                                    );
                                } else {
                                    requirements = [];
                                    if (serverid !== "none") {
                                        ...
                                        if (!server) {
                                            message.channel.send(
                                                new Discord.MessageEmbed()
                                                    .setTitle('Greesy | Giveaway')
                                                    .setColor("RANDOM")
                                                    .setDescription(`Please Write A Valid Server ID.`)//Could not get the server id on the **third** argument
                                            );
                                            return;
                                        } else {
                                            let invite = await (server.channels.cache.filter(c => c.type === 'text').find(x => x.position == 0)).createInvite(
                                                ...
                                                `Giveaway command runned by : ${message.author.tag}`
                                            )
                                            requirements.push(`**Must Join Server**: [${server.name}](${invite})`);
                                        };
                                    };
                                    if (rolevar !== "none") {
                                        ...
                                        if (!role) {
                                            message.channel.send(
                                                new Discord.MessageEmbed()
                                                    .setTitle('Greesy | Giveaway')
                                                    .setColor("RED")
                                                    .setDescription(`You Don't Write Role **ID**.`)
                                            );
                                            return;
                                        } else {
                                            requirements.push(`Must Have This Role: <@&${role.id}>`);
                                        }
                                    };
                                        ...
                                        if (isNaN(messagecount)) {
                                            message.channel.send(
                                                new Discord.MessageEmbed()
                                                    .setTitle('Greesy | Giveaway')
                                                    .setColor("RANDOM")
                                                    .setDescription(`Please Write A Number! .`)
                                            );
                                            return;
                                        } else {
                                            if (messagecount < 1 || messagecount > 1000) {
                                                message.channel.send(
                                                    new Discord.MessageEmbed()
                                                        .setTitle('Greesy | Giveaway')
                                                        .setColor("RED")
                                                        .setDescription(`It must be 1 or 1000 digits! .`)
                                                );
                                                return;
                                            } else {
                                                ...
                                                requirements.push(`Must Have \`${messagecount}\` Messages.`);
                                            };
                                        };
                                    } else {
                                        msgcounttest = null;
                                    ...
                                    let embed = new Discord.MessageEmbed()
                                        .setTitle('Greesy | Giveaway! ' + prize[0])
                                        .setColor("YELLOW")
                                        .setDescription(`You Need to Press the 🎉 Response to Participate .`)
                                        .addField("Winner(s)", "There will be **" + winners + "** winners.")
                                        .addField("Requirements", (await requirements).length == 0 ? "None!" : requirements.join("\n") + (!prize[1] ? "" : "\n" + prize.join(" | ").slice(prize[0].length + 3)))
                                        .setFooter("Ends  " + ms(time, {long:true}))
                                                ...
                                                if (await msg2) {
                                                    ...
                                                    if (react.length == 0) {
                                                        await msg2.edit(
                                                            new Discord.MessageEmbed()
                                                                .setTitle('Greesy | Giveaway! ' + prize[0])
                                                                .setDescription(`Giveaway  It's over.`)
                                                                .addField("Winner(s) ", "No winner(s).")
                                                                .addField("Requirement(s)", (await requirements).length == 0 ? "None!" : requirements.join("\n"))
                                                                .setFooter("There Could Have been up to " + winners+ " winner(s.) ")
                                                        )
                                                        message.channel.send(":tada: | **The giveaway has Ended.** Our winner(s) are: No winner(s).")
                                                    } else {
                                                        let users = [];
                                                        ...
                                                        await msg2.edit(
                                                            new Discord.MessageEmbed()
                                                                .setTitle('Greesy | Giveaway! ' + prize[0])
                                                                .setDescription(`Giveaway it's over.`)
                                                                .addField("Winner(s)", users.length !== 0 ? users.join("\n") : "No winner(s).")
                                                                .addField("Requirement(s)", (await requirements).length == 0 ? "None!" : requirements.join("\n"))
                                                                .setFooter("There Could Have been up to " + winners + " winner(s) ")
                                                        )
                                                        message.channel.send(":tada: **The giveaway has Ended.** Our winner(s) are:" + (users.length == 0 ? " No winners." : "\n- " + users.join("\n- ")))
                                                    }
                                                }
                                            }
                                        , time);
